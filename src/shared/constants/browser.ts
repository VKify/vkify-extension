 ...
 *
 * Значения: 'chrome' | 'firefox'. По умолчанию (вне сборки, напр. в
 * тестах) — 'chrome'.
 *
 ...
 * уместнее. Различия нужны только для Firefox; Chrome представляет Chromium.
 */
...
function resolveBrowser(): 'chrome' | 'firefox' {
  try {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (typeof __VKIFY_BROWSER__ === 'string' && __VKIFY_BROWSER__) {
      return __VKIFY_BROWSER__ as 'chrome' | 'firefox';
    }
