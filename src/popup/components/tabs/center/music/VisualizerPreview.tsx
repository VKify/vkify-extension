...
export default function VisualizerPreview({ settings, animated = true, thumbnail = false, className = '', onOffsetChange }: {
  ...
  useEffect(() => {
    ...
    const draw = (now: number): void => {
      ...
      if (g) {
        ...
        renderer.draw(g, width, height, preview, [current.color, current.colorMode === 'custom' ? current.color : current.color2], reduced.matches);
      }
  ...
  return <canvas ref={canvasRef} aria-hidden="true" className={className}
    onPointerDown={(event) => {
      ...
      dragRef.current = { id: event.pointerId, x: event.clientX, y: event.clientY, offsetX: settings.offsetX, offsetY: settings.offsetY };
      event.currentTarget.setPointerCapture(event.pointerId);
      event.preventDefault();
    }}
    onPointerMove={(event) => {
      const drag = dragRef.current;
      if (!drag || drag.id !== event.pointerId || !onOffsetChange) return;
      const rect = event.currentTarget.getBoundingClientRect();
      const clamp = (value: number): number => Math.max(-100, Math.min(100, Math.round(value)));
      onOffsetChange(clamp(drag.offsetX + (event.clientX - drag.x) / Math.max(1, rect.width) * 100), clamp(drag.offsetY + (event.clientY - drag.y) / Math.max(1, rect.height) * 100));
    }}
    onPointerUp={endDrag} onPointerCancel={endDrag} onLostPointerCapture={() => { dragRef.current = null; }}
    style={{ touchAction: onOffsetChange ? 'none' : undefined, filter: !thumbnail && settings.blur ? `blur(${settings.blur}px)` : undefined }} />;
}
