<div align="center">
  ...
  **Расширение для Chrome, Firefox и Opera, которое делает ВКонтакте удобнее, красивее и приватнее**
  ...
  ![Version](https://img.shields.io/badge/версия-1.8.3-blue?style=flat-square)
...
VKify собирает в одном расширении всё, чего обычно не хватает во ВКонтакте: оформление под себя, чистую ленту без рекламы, приватность переписки, скачивание медиа и инструменты для сообщений. Настройки открываются в попапе (10 вкладок) или прямо на странице — `vk.ru/vkify_settings`. По `Ctrl/Cmd + K` работает поиск по всем функциям.

Оформление и скрытие блоков применяются **мгновенно и без перезагрузки**: изменения видны на странице ещё в момент перетаскивания ползунка, синхронизируются между всеми открытыми вкладками, а при следующей загрузке страницы оформление подхватывается ещё до первой отрисовки — без «вспышки» ванильного VK. После переключения API-фильтра рекламы расширение предложит обновить активную страницу ВК сейчас или позже.
...
- Блокировка боковых рекламных баннеров, трекеров и аналитики
- API-фильтрация рекламных записей в ленте, сообществах, профилях и поиске, включая предварительно загруженные данные страницы
- Общий список стоп-слов и исключений для API и дополнительного DOM-фильтра: поиск по вхождению без учёта регистра, исключения важнее стоп-слов. Встроенная API-блокировка рекламы остаётся независимой от исключений по словам
- DOM-фильтр работает только по пользовательским словам, выключен по умолчанию и не входит в общую кнопку защиты. Изменения правил сразу применяются к странице и новым ответам API; DOM-фильтр возвращает записи, которые больше не нужно скрывать
- **Реклама и рекомендации по разделам** — девять независимых переключателей: лента, игры, маркет, звонки, меню профиля, мессенджер, музыка, сообщества и Яндекс Браузер в левом меню
- Каждый переключатель подробно описывает скрываемые блоки. В сообществах скрывается блок «Похожие сообщества» и следующий элемент; в музыке — баннеры подписки, промопредложения и «Слушайте друг друга», без блокировки звуковых рекламных вставок
- При установке и сбросе все девять категорий скрыты. Общая защита и быстрые действия учитывают их состояние; миграции и импорт старых копий сохраняют прежний выбор пользователя
...
- Карточка защиты со статистикой за всё время: всего блокировок, рекламные записи и трекеры. Журнал с фильтрами, пагинацией и JSON-снимком API-блокировок

Полный список изменений v1.8.3, включая исправления эквалайзера и правой колонки ленты: [журнал релиза](.github/release-notes/v1.8.3.md). Готовые архивы Chrome, Firefox и Opera и подписанный Firefox XPI: [GitHub Release](https://github.com/VKify/vkify-extension/releases/tag/v1.8.3).
...
- Карусель «возможные друзья», эмодзи-статус у имени
- Промо-блок на профиле
...
- **Сообщения** — быстрое копирование последнего сообщения, экспорт диалога в JSON, TXT, HTML и ZIP, а также всего диалога или выбранных сообщений в PDF с вложениями, шаблоны и заметки
...
- **Видео** — скачивание с выбором качества от 1080p до 240p: страницы vkvideo.ru и модальный плеер vk.ru (ссылки вида `?z=video-…`)
...
- Встроенная страница настроек на vk.ru и пункт «Настройки VKify» в меню профиля
...
- Или откройте `vk.ru/vkify_settings` (либо пункт «Настройки VKify» в меню профиля) — те же настройки прямо на странице VK.
...
- **embed** — контент-скрипт, который встраивает тот же интерфейс настроек прямо в страницу VK (`vk.ru/vkify_settings` и пункт меню профиля)
...
Набор намеренно минимальный; каждое разрешение обосновано:

| Разрешение | Зачем |
|---|---|
| `storage` | Настройки и данные функций в `chrome.storage.local`. |
| `tabs` | Найти/перезагрузить вкладки vk.ru, открыть попап на нужной подстранице. |
| `alarms` | Периодические задачи слежки (онлайн/профиль). |
| `notifications` | Уведомления слежки. |
| `downloads` | Сохранение аудио/видео/фото. |
| `scripting` | **On-demand** инъекция `audio-encoder.js` (hls.js + lamejs) в ISOLATED-мир вкладки только при старте скачивания аудио — чтобы тяжёлые библиотеки не грузились в `content.js` на каждой странице (`document_start`). |

`scripting` НЕ добавляет отдельного предупреждения при установке в Chrome: оно
перекрывается уже запрошенными `host_permissions` на vk.ru. Сделать его
`optional` нельзя без ухудшения UX: скачивание запускается из content-скрипта, а
`chrome.permissions.request()` там недоступен (нужен жест на странице расширения).
...
└── src/
    ...
    │   ├── embed/                    # Настройки прямо на странице VK (vk.ru/vkify_settings)
    ...
    │   │   ├── settings-page/        # Интеграция vk.ru/vkify_settings
...
- **Реклама и миграции** — API и HTML-prefetch, слова и исключения, восстановление DOM-записей, диалог перезагрузки, перенос настроек и импорт старых копий; полный набор v1.8.3 — 500 тестов
