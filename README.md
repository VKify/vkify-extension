...
### 🛡️ Реклама (`Реклама`)
- Реклама в ленте (блокировка на уровне API)
- Рекламные баннеры в боковой панели
- Трекеры и аналитика (Яндекс.Метрика, Google Analytics, Facebook Pixel и др.)
- Все три фильтра включены по умолчанию при первой установке
...
│   │   │   ├── ads-blocking/
│   │   │   │   └── index.ts                # Блокировка рекламы и трекеров
│   │   │   ├── automation/
...
│   │   │   ├── ad-feed-blocker.ts          # Блокировка рекламы в ленте (API)
│   │   │   ├── anti-tracking.ts            # Антифингерпринтинг
│   │   │   ├── injected-vk-api.ts          # Доступ к внутреннему API VK
│   │   │   ├── spy.ts                      # Перехват WebSocket-событий VK
│   │   │   ├── tracker-blocker.ts          # Блокировка трекеров
