...
- **Скачивание сторис** с vk.com — при просмотре сторис кнопка-иконка
  появляется прямо в панели управления плеера, рядом с кнопкой «⋯»: фото-сторис
  скачиваются в JPEG одним кликом, видео-сторис — с выпадающим пикером качества;
  кнопка позиционируется через `data-testid` без привязки к генерируемым
  CSS-классам; опрашивает `location.search` каждые 300 мс для надёжного
  обнаружения навигации (VK меняет только `?w=`, не обновляя `<title>`);
...
- **Скачивание клипов** на vk.com и vkvideo.ru — кнопка встраивается в правую
  ...
  по `owner_id`
- **Скачивание фото и альбомов** на vk.com — кнопка «Скачать» появляется в
  фотоoverlay (`.pv_bottom_actions`, рядом с «Поделиться/Удалить») и
  скачивает оригинал максимального разрешения через `photos.getById`;
  идентификатор фото берётся из CSS-класса `_like_photo<owner>_<id>`,
  фолбэк — `data-options` атрибут. На странице альбома (`/album<owner>_<id>`)
  кнопка «Скачать альбом» в правой панели заголовка постранично запрашивает
  `photos.get` (по 1000 за запрос) и сохраняет все фото в подпапку
  `vkify-album-<owner>_<id>/photo_NNN_<id>.jpg` с прогресс-индикатором
