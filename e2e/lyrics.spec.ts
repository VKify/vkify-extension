...
test('lyrics settings preview, independent saves and TXT/LRC downloads', async ({}, testInfo) => {
  ...
  try {
    ...
    const bundle = await build({ stdin: { resolveDir: process.cwd(), loader: 'tsx', contents: `
      ...
      import MusicVisualizerPage from './src/popup/components/tabs/center/music/MusicVisualizerPage.tsx';
    ...
    expect(await page.evaluate('JSON.parse(window.state.getState().settings.music_lyrics_settings).lyricsAlignment')).toBe('right');
    const auto = page.getByRole('switch', { name: 'Автоматическое смещение страницы', exact: true });
    await auto.click();
    ...
    await page.getByText('Настройки строк', { exact: true }).click();
    ...
    await page.evaluate('window.state.getState().saveSetting("music_lyrics",false);window.showVisualizer()');
